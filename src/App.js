...
function App() {
  ...
  const pricePerProducts = useMemo(() => usings.reduce((acc, using) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [fetchedPersons, fetchedProducts, fetchedUsings] = await Promise.all([
          firebase.getPersons(),
  ...
  return (
    <div className="container">
      ...
      <Button className="add-product" onClick={() => openProductModal('add')} variant="contained">Добавить продукт</Button>
      <Table persons={persons} products={products} usings={usings} pricePerProducts={pricePerProducts} onToggleCheckbox={onToggleCheckbox} onEditProduct={(product) => {
      ...
      <Modal open={productModal} onClose={() => setProductModal(false)} >
        <ProductForm onSubmit={onProductFormSubmit} action={productFormAction} product={productToChange} />
      </Modal>
    </div>
  )
}
